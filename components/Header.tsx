    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <a href="#home" className="flex items-center space-x-2">
          <div className={`w-10 h-10 flex items-center justify-center rounded-lg ${isScrolled ? 'bg-blue-900 text-white' : 'bg-white text-blue-900'}`}>
          ...
          <div className="flex flex-col">
             ...
             <span className={`text-xs font-semibold tracking-widest ${isScrolled ? 'text-orange-600' : 'text-orange-400'}`}>COPERTURE</span>
          </div>
        </a>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href}
              className={`text-sm font-semibold transition-colors hover:text-orange-500 ${isScrolled ? 'text-gray-700' : 'text-white'}`}
            >
              {link.name}
            </a>
          ))}
          <a 
            href="#preventivo" 
            className="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-orange-600/20 transition-all transform hover:-translate-y-0.5 active:scale-95"
          >
            PREVENTIVO GRATIS
          </a>
        </nav>
      ...
      {/* Mobile Menu */}
      <div 
        className={`fixed inset-0 bg-blue-950 z-40 transition-transform duration-500 md:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex flex-col items-center justify-center h-full space-y-8 p-6">
          ...
          <div className="pt-4 flex flex-col items-center space-y-6 w-full max-w-xs">
            <a 
              href="#preventivo" 
              onClick={() => setIsMenuOpen(false)}
              className="w-full bg-orange-600 text-white text-center py-4 rounded-xl text-lg font-bold"
            >
              Richiedi Preventivo
            </a>
            <a 
